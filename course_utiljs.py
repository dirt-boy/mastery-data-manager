...
# Code follows:
var.registers(['getRoster', 'formatAll', 'getCourseList'])
@Js
def PyJsHoisted_getCourseList_(courseData, prop, this, arguments, var=var):
    ...
    var.registers(['i', 'prop', 'titles', 'j', 'courseData'])
    var.put('titles', Js({}))
    #for JS loop
    var.put('i', Js(0.0))
...
def PyJsHoisted_formatAll_(response, this, arguments, var=var):
    ...
    return Js({'courses':var.get('getCourseList')(var.get('response'), Js('courses')),'courseWork':var.get('getCourseList')(var.get('response'), Js('courseWork')),'roster':var.get('getRoster')(var.get('response'))})
